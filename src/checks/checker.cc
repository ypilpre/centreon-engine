...
void checker::run(
  ...
  shared_ptr<commands::command>
    cmd(cmd_set.get_command(hst->get_check_command()->name));
...
void checker::run(
  ...
  if (check_service_check_viability(
    ...
    throw (checks_viability_failure() << "Check of service '"
           << svc->get_description() << "' on host '"
           << svc->get_host_name() << "' is not viable");
  ...
  int res(broker_service_check(
            ...
            start_time,
            end_time,
            svc->get_check_command_args().c_str(),
            svc->get_latency(),
            0.0,
            0,
            false,
  ...
  // Service check was override by NEB module.
    ...
    logger(dbg_functions, basic)
      ...
      << svc->get_description() << "' on host '"
      << svc->get_host_name() << "' so we'll bail out";
    return ;
  }
  ...
  nagios_macros macros;
  memset(&macros, 0, sizeof(macros));
  grab_host_macros_r(&macros, svc->get_host());
  grab_service_macros_r(&macros, svc);
  get_raw_command_line_r(
    &macros,
    svc->get_check_command(),
    svc->get_check_command_args().c_str(),
    NULL,
    0);
  ...
  check_result_info.output_file_fp = NULL;
  check_result_info.output_file = NULL;
  check_result_info.host_name = string::dup(svc->get_host_name().c_str());
  check_result_info.service_description = string::dup(svc->get_description().c_str());
  check_result_info.latency = latency;
  check_result_info.next = NULL;
  ...
  commands::set& cmd_set(commands::set::instance());
  shared_ptr<commands::command>
    cmd(cmd_set.get_command(svc->get_check_command()->name));
  std::string processed_cmd(cmd->process_cmd(&macros));
  char* processed_cmd_ptr(string::dup(processed_cmd));
...
void checker::run_sync(
  ...
  broker_host_check(
    ...
    hst->get_current_state(),
    NULL,
    hst->get_output().c_str(),
    hst->get_long_output().c_str(),
    hst->get_perfdata().c_str(),
    NULL);
...
int checker::_execute_sync(host* hst) {
  ...
  // Debug messages.
  logger(dbg_commands, more) << "Raw host check command: "
    << hst->get_check_command()->command_line;
  ...
  if (!hst->get_check_command()) {
    hst->set_output("(Host assumed to be UP)");
    res.exit_code = STATE_OK;
  }

  // Make sure we have some data.
  if (hst->get_output().empty())
    hst->set_output("(No output returned from host check)");

  // Replace semicolons with colons in plugin output
  // (but not performance data).
  {
    std::string escaped_output(hst->get_output());
    std::replace(escaped_output.begin(), escaped_output.end(), ';', ':');
    hst->set_output(escaped_output);
  }
