...
void applier::host::apply(
  ...
  for (list_host::const_iterator it(lst.begin()), end(lst.end());
       ...
       ++it) {
    try {
      ::host& hst(*configuration::applier::state::instance().hosts_find((*it)->host_name()));
...
void applier::host::_update(
  ...
  if (obj.get_retain_nonstate_info()) {
    ...
    if (state.check_period().is_set()
      ...
      try {
        configuration::applier::host hstaplr;
        hstaplr.resolve_check_period(obj, *state.check_period());
      }
      catch (not_found const& e) {
        // Target does not exist, remove modified attribute flag.
        (void)e;
    ...
    if (state.notification_period().is_set()
      ...
      try {
        configuration::applier::host hstaplr;
        hstaplr.resolve_notification_period(
          obj,
    ...
    if (state.event_handler().is_set()
      ...
      try {
        configuration::applier::host hstaplr;
        hstaplr.resolve_event_handler(obj, *state.event_handler());
      }
      catch (not_found const& e) {
        // Target does not exist, remove modified attribute flag.
        (void)e;
...
//        obj.get_last_notification()));
  ...
  // since restart).
  if (obj.get_current_state() != HOST_UP
