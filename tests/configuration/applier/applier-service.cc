...
// And without a check command
// Then the applier add_object throws an exception.
TEST_F(ApplierService, NewServiceWithoutCommand) {
  configuration::applier::host hst_aply;
  configuration::applier::service svc_aply;
  configuration::service svc;
  configuration::host hst;
...
TEST_F(ApplierService, NewServiceFromConfig) {
  ...
  umap<std::pair<unsigned int, unsigned int>, com::centreon::shared_ptr<service_struct> > const&
    sm(configuration::applier::state::instance().services());
  ASSERT_EQ(sm.size(), 1);
  ASSERT_EQ(sm.begin()->first.first, 12);
  ASSERT_TRUE(sm.begin()->first.second == 18);
...
TEST_F(ApplierService, ServicesEquality) {
  ...
  hst_aply.add_object(hst);
  ASSERT_TRUE(svc.parse("hosts", "test_host"));
  ASSERT_TRUE(svc.parse("service_description", "test description1"));
  ASSERT_TRUE(svc.parse("service_id", "18"));
  cmd.parse("command_line", "echo 1");
  svc.parse("check_command", "cmd");
  cmd_aply.add_object(cmd);
  ASSERT_TRUE(svc.parse("acknowledgement_timeout", "21"));
  svc_aply.add_object(svc);
  svc_aply.expand_objects(*config);
  umap<std::pair<unsigned int, unsigned int>, com::centreon::shared_ptr<service_struct> > const&
    sm(configuration::applier::state::instance().services());
  ASSERT_EQ(sm.size(), 1);
  configuration::service svc1(svc);
  ASSERT_EQ(svc, svc1);
  ASSERT_TRUE(svc1.parse("recovery_notification_delay", "120"));
  ASSERT_TRUE(svc < svc1);
  ASSERT_TRUE(svc.parse("retry_interval", "120"));
  ASSERT_TRUE(svc1 < svc);
}
...
// When Modifying a configuration changes,
// Then the '<' effect on configurations.
TEST_F(ApplierService, ServicesDescriptionModified) {
  configuration::applier::host hst_aply;
  configuration::applier::service svc_aply;
  configuration::applier::command cmd_aply;
  configuration::service svc;
  ...
  umap<std::pair<unsigned int, unsigned int>, com::centreon::shared_ptr<service_struct> > const&
    sm(configuration::applier::state::instance().services());
  ASSERT_EQ(sm.size(), 1);
  com::centreon::shared_ptr<service_struct> s1(sm.begin()->second);
  ASSERT_TRUE(strcmp(s1->description, "test description2") == 0);
}
