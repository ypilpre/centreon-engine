...
TEST_F(ApplierServicegroup, ResolveServicegroup) {
  ...
  configuration::host hst("test_host");
  aply_hst.add_object(hst);
  configuration::service svc;
  ASSERT_TRUE(svc.parse("description", "test"));
...
TEST_F(ApplierServicegroup, RemoveServicegroupFromConfig) {
  ...
  configuration::applier::host aply_hst;
  configuration::applier::servicegroup aply_grp;
  configuration::servicegroup sg("test_group");
  configuration::host hst("test_host");
  aply_hst.add_object(hst);
  configuration::service svc;
  ASSERT_TRUE(svc.parse("description", "test"));
  ASSERT_TRUE(svc.parse("host", "test_host"));
  aply_svc.add_object(svc);
  sg.parse("members", "test_host,test");
  aply_grp.add_object(sg);
  aply_grp.expand_objects(*config);
  aply_grp.resolve_object(sg);

  /* The second resolve_object clears members before re-creating them.
   * broker events are then sent. */
  aply_grp.resolve_object(sg);
  ASSERT_TRUE(sg.members().size() == 1);

  aply_grp.remove_object(sg);
  ASSERT_TRUE(
    configuration::applier::state::instance().servicegroups().empty());
}
